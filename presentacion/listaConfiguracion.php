...
$data=$objConfig->consultarConfiguracion($_GET['codigo'], $_GET['descripcion'], $_GET['estado'], $inicio, $cantidad);
$total_datos=$objConfig->consultarConfiguracion($_GET['codigo'], $_GET['descripcion'], $_GET['estado'], $inicio, $cantidad, true);
...
<div class="table-responsive">
	<table class="table table-bordered table-vertical-center table-hover table-sm font-size-sm">
	    ...
	    <tbody>
	        ...
	        <tr class="<?= $class ?>">
	            <td><?= $fila['codigo'] ?></td>
                <td class="font-weight-boldest"><?= $fila['idconfig'] ?></td>
	            <td><?= $fila['descripcion'] ?></td>
	            <td><?= $fila['valor'] ?></td>
	            <td><?= $fila['observacion'] ?></td>
	            <td><?php if($fila['estado']=='N'){echo "ACTIVO";}else{echo "ANULADO";};?></td>
	            <td class="text-center w-xl-70px">
	            	<button type="button" class="btn btn-light-primary font-weight-bold btn-sm" data-toggle="tooltip" data-placement="top" title="Editar Perfil" href="javascript:void(0)" onclick="editar<?= $sufijo ?>('<?php echo $fila['codigo'];?>')" ><li class="fa fa-edit"></li></button>
	            </td>
				    ...
				    <div class="dropdown">
						...
						<div class="dropdown-menu dropdown-menu-sm" style="">
							<ul class="navi">
								...
								<li class="navi-item">
									<a class="navi-link" href="#" onclick="cambiarEstado<?= $sufijo ?>(<?= $fila['codigo'] ?>,'A')">
								...
								<li class="navi-item">
									<a class="navi-link" href="#" onclick="cambiarEstado<?= $sufijo ?>(<?= $fila['codigo'] ?>,'N')">
										<span class="navi-icon">
								...
								<li class="navi-item">
									<a class="navi-link" href="#" onclick="cambiarEstado<?= $sufijo ?>(<?= $fila['codigo'] ?>,'E')">
										<span class="navi-icon">
...
function editar<?= $sufijo ?>(codigo){
	ViewModal('presentacion/mantConfiguracion','codigo='+codigo+'&accion=MODIFICAR&idopcion=<?= $idopcion ?>&nivel=<?= $nivel + 1 ?>&puedeeditar=<?= $puedeeditar ?>','divmodal<?= $nivel ?>','Edición de Configuración');
}

function cambiarEstado<?= $sufijo ?>(codigo, estado){
	let msj="";
	if(estado=="A"){msj="¿Esta seguro de anular la Configuración?";}
	if(estado=="N"){msj="¿Esta seguro de activar la Configuración?";}
	if(estado=="E"){msj="¿Esta seguro de eliminar la Configuración?";}
	confirm(msj,'ProcesoCambiarEstado<?= $sufijo ?>("'+codigo+'","'+estado+'")');
}

function ProcesoCambiarEstado<?= $sufijo ?>(codigo,estado){
	...
	$.ajax({
		...
		data: {accion: "CAMBIAR_ESTADO_CONFIGURACION",
				'codigo': codigo,
				'estado': estado,
				'idopcion': <?= $idopcion ?>
			}
