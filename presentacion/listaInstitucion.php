...
$data=$objIns->consultarInstitucion($_GET['nombre'], $_GET['documento'], $_GET['estado'], $inicio, $cantidad);
$total_datos=$objIns->consultarInstitucion($_GET['nombre'], $_GET['documento'], $_GET['estado'], $inicio, $cantidad, true);
...
<div class="table-responsive">
	<table class="table table-bordered table-vertical-center table-hover table-sm font-size-sm">
	    ...
	    <tbody>
	    	...
	    	?>
	        <tr class="<?= $class ?>">
	            <td class="font-weight-boldest"><?= $fila['idinstitucion'] ?></td>
	            <td><?= $fila['ruc'] ?></td>
	            <td><?= trim($fila['nombre']) ?></td>
	            <td><?= $fila['direccion'] ?></td>
	            <td><?= $fila['nombrerepresentante'] ?></td>
	            <td><?php if($fila['estado']=='N'){echo "ACTIVO";}else{echo "ANULADO";};?></td>
	            <td>
	            	<button type="button" class="btn btn-light-primary font-weight-bold btn-sm" data-toggle="tooltip" data-placement="top" title="Editar Institución" href="javascript:void(0)" onclick="editar<?= $sufijo ?>('<?php echo $fila['idinstitucion'];?>')" ><li class="fa fa-edit"></li></button>
	            </td>
	            <td>
				    <div class="dropdown">
						...
						<div class="dropdown-menu dropdown-menu-sm" style="">
							<ul class="navi">
								...
								<li class="navi-item">
									<a class="navi-link" href="#" onclick="cambiarEstado<?= $sufijo ?>(<?= $fila['idinstitucion'] ?>,'A')">
								...
								<li class="navi-item">
									<a class="navi-link" href="#" onclick="cambiarEstado<?= $sufijo ?>(<?= $fila['idinstitucion'] ?>,'N')">
										<span class="navi-icon">
								...
								<li class="navi-item">
									<a class="navi-link" href="#" onclick="cambiarEstado<?= $sufijo ?>(<?= $fila['idinstitucion'] ?>,'E')">
										<span class="navi-icon">
...
function editar<?= $sufijo ?>(idinstitucion){
	ViewModal('presentacion/mantInstitucion','idinstitucion='+idinstitucion+'&accion=MODIFICAR&idopcion=<?= $idopcion ?>&nivel=<?= $nivel + 1 ?>&puedeeditar=<?= $puedeeditar ?>','divmodal<?= $nivel ?>','Edición de Institución');
}


function cambiarEstado<?= $sufijo ?>(idinstitucion, estado){
	let msj="";
	if(estado=="A"){msj="¿Esta seguro de anular la Institución?";}
	if(estado=="N"){msj="¿Esta seguro de activar la Institución?";}
	if(estado=="E"){msj="¿Esta seguro de eliminar la Institución?";}
	confirm(msj,'ProcesoCambiarEstado<?= $sufijo ?>("'+idinstitucion+'","'+estado+'")');
}

function ProcesoCambiarEstado<?= $sufijo ?>(idinstitucion,estado){
	...
	$.ajax({
		...
		data: {accion: "CAMBIAR_ESTADO_INSTITUCION",
