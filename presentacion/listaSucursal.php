...
$data=$objSuc->consultarSucursal($_GET['nombre'], $_GET['idinstitucion'], $_GET['estado'], $inicio, $cantidad);
$total_datos=$objSuc->consultarSucursal($_GET['nombre'], $_GET['idinstitucion'], $_GET['estado'], $inicio, $cantidad, true);
...
<div class="table-responsive">
	<table class="table table-bordered table-vertical-center table-hover table-sm font-size-sm">
	    ...
	    <tbody>
	    	...
	    	?>
	        <tr class="<?= $class ?>">
	            <td class="font-weight-boldest"><?= $fila['idsucursal'] ?></td>
	            <td><?= trim($fila['nombre']) ?></td>
	            <td><?= $fila['direccion'] ?></td>
	            <td><?= $fila['institucion'] ?></td>
	            <td><?php if($fila['estado']=='N'){echo "ACTIVO";}else{echo "ANULADO";};?></td>
	            <td>
	            	<button type="button" class="btn btn-light-primary font-weight-bold btn-sm" data-toggle="tooltip" data-placement="top" title="Editar Sucursal" href="javascript:void(0)" onclick="editar<?= $sufijo ?>('<?php echo $fila['idsucursal'];?>')" ><li class="fa fa-edit"></li></button>
	            </td>
	            <td>
				    <div class="dropdown">
						...
						<div class="dropdown-menu dropdown-menu-sm" style="">
							<ul class="navi">
								...
								<li class="navi-item">
									<a class="navi-link" href="#" onclick="cambiarEstado<?= $sufijo ?>(<?= $fila['idsucursal'] ?>,'A')">
								...
								<li class="navi-item">
									<a class="navi-link" href="#" onclick="cambiarEstado<?= $sufijo ?>(<?= $fila['idsucursal'] ?>,'N')">
										<span class="navi-icon">
								...
								<li class="navi-item">
									<a class="navi-link" href="#" onclick="cambiarEstado<?= $sufijo ?>(<?= $fila['idsucursal'] ?>,'E')">
										<span class="navi-icon">
...
function editar<?= $sufijo ?>(idsucursal){
	ViewModal('presentacion/mantSucursal','idsucursal='+idsucursal+'&accion=MODIFICAR&idopcion=<?= $idopcion ?>&nivel=<?= $nivel + 1 ?>&puedeeditar=<?= $puedeeditar ?>','divmodal<?= $nivel ?>','Edición de Sucursal');
}


function cambiarEstado<?= $sufijo ?>(idsucursal, estado){
	let msj="";
	if(estado=="A"){msj="¿Esta seguro de anular la Sucursal?";}
	if(estado=="N"){msj="¿Esta seguro de activar la Sucursal?";}
	if(estado=="E"){msj="¿Esta seguro de eliminar la Sucursal?";}
	confirm(msj,'ProcesoCambiarEstado<?= $sufijo ?>("'+idsucursal+'","'+estado+'")');
}

function ProcesoCambiarEstado<?= $sufijo ?>(idsucursal,estado){
