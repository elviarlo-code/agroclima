...
function verPagina<?php echo $sufijo;?>(loading=1){
	...
	let filtro='nombre='+nombre+'&nivel='+nivel+'&idcultivo=<?= $idcultivo ?>&idcampania=<?= $idcampania ?>&cultivo=<?= $rowCultivo['nombre'] ?>';
	...
	setRun('presentacion/viewFenologiaCampaniaLista',filtro+extrapermiso,'divLista<?php echo $sufijo;?>','divLista<?php echo $sufijo;?>',loading);	
}

verPagina<?php echo $sufijo;?>(0);

function NuevoRegistro<?= $sufijo ?>(){
	ViewModal('presentacion/viewFenologiaCampaniaMant','accion=NUEVO&idopcion=<?= $idopcion ?>&nivel=<?= $nivel + 1 ?>&idcultivo=<?= $idcultivo ?>&idcampania=<?= $idcampania ?>','divmodal<?= $nivel ?>','REGISTRAR FENOLOGIA - <?= htmlentities($rowCultivo['nombre']) ?>',0);
}
