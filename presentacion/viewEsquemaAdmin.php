				...
				<div class="row">
					<div class="col-md-5">
						<div class="input-group input-group-sm">
							...
							<input type="text" name="busquedaTerreno<?= $sufijo; ?>" id="busquedaTerreno<?= $sufijo; ?>" value="<?= $nombre ?>" class="form-control" readonly="readonly" autocomplete="off">
						</div>
					</div>
					<div class="col-md-7">
						<div class="input-group input-group-sm">
...
function verPagina<?php echo $sufijo;?>(loading=1){
	...
	let filtro='nombre='+nombre+'&nivel='+nivel+'&idterreno=<?= $idterreno ?>&terreno=<?= $rowTerreno['descripcion'] ?>';
	...
	setRun('presentacion/viewEsquemaLista',filtro+extrapermiso,'divLista<?php echo $sufijo;?>','divLista<?php echo $sufijo;?>',loading);	
}

verPagina<?php echo $sufijo;?>(0);

function NuevoRegistro<?= $sufijo ?>(){
	ViewModal('presentacion/viewEsquemaMant','accion=NUEVO&idopcion=<?= $idopcion ?>&nivel=<?= $nivel + 1 ?>&idterreno=<?= $idterreno ?>','divmodalmediano','REGISTRAR ESQUEMA',0);
}
