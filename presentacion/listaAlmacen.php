...
$data=$objAlm->consultarAlmacen($_GET['nombre'], $_GET['idinstitucion'], $_GET['idsucursal'], $_GET['estado'], $inicio, $cantidad);
$total_datos=$objAlm->consultarAlmacen($_GET['nombre'], $_GET['idinstitucion'], $_GET['idsucursal'], $_GET['estado'], $inicio, $cantidad, true);
...
<div class="table-responsive">
	<table class="table table-bordered table-vertical-center table-hover table-sm font-size-sm">
	    ...
	    <tbody>
	    	...
	    	?>
	        <tr class="<?= $class ?>">
	            <td class="font-weight-boldest"><?= $fila['idalmacen'] ?></td>
	            <td><?= trim($fila['descripcion']) ?></td>
	            <td><?= $fila['direccion'] ?></td>
	            <td><?= $fila['institucion'] ?></td>
	            <td><?= $fila['sucursal'] ?></td>
	            <td><?php if($fila['estado']=='N'){echo "ACTIVO";}else{echo "ANULADO";};?></td>
	            <td>
	            	<button type="button" class="btn btn-light-primary font-weight-bold btn-sm" data-toggle="tooltip" data-placement="top" title="Editar Almacén" href="javascript:void(0)" onclick="editar<?= $sufijo ?>('<?php echo $fila['idalmacen'];?>')" ><li class="fa fa-edit"></li></button>
	            </td>
	            <td>
				    <div class="dropdown">
						...
						<div class="dropdown-menu dropdown-menu-sm" style="">
							<ul class="navi">
								...
								<li class="navi-item">
									<a class="navi-link" href="#" onclick="cambiarEstado<?= $sufijo ?>(<?= $fila['idalmacen'] ?>,'A')">
								...
								<li class="navi-item">
									<a class="navi-link" href="#" onclick="cambiarEstado<?= $sufijo ?>(<?= $fila['idalmacen'] ?>,'N')">
										<span class="navi-icon">
								...
								<li class="navi-item">
									<a class="navi-link" href="#" onclick="cambiarEstado<?= $sufijo ?>(<?= $fila['idalmacen'] ?>,'E')">
										<span class="navi-icon">
...
function editar<?= $sufijo ?>(idalmacen){
	ViewModal('presentacion/mantAlmacen','idalmacen='+idalmacen+'&accion=MODIFICAR&idopcion=<?= $idopcion ?>&nivel=<?= $nivel + 1 ?>&puedeeditar=<?= $puedeeditar ?>','divmodal<?= $nivel ?>','Edición de Almacén');
}


function cambiarEstado<?= $sufijo ?>(idalmacen, estado){
	let msj="";
	if(estado=="A"){msj="¿Esta seguro de anular el Almacén?";}
	if(estado=="N"){msj="¿Esta seguro de activar el Almacén?";}
	if(estado=="E"){msj="¿Esta seguro de eliminar el Almacén?";}
	confirm(msj,'ProcesoCambiarEstado<?= $sufijo ?>("'+idalmacen+'","'+estado+'")');
}

function ProcesoCambiarEstado<?= $sufijo ?>(idalmacen,estado){
	...
	$.ajax({
		...
		data: {accion: "CAMBIAR_ESTADO_ALMACEN",
				'idalmacen': idalmacen,
				'estado': estado,
				'idopcion': <?= $idopcion ?>
			}
