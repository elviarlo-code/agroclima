		...
		<div class="card card-custom gutter-b">
			<div class="card-header" style="min-height: 50px">
				<div class="card-title">
					...
					<h3 class="card-label">Sucursal
					<small>Busqueda de Sucursal</small></h3>
				</div>
			</div>
			<div class="card-body">
				<div class="row">
					...
					<div class="col-md-4">
						<div class="input-group input-group-sm">
							...
							<select class="form-control" name="busquedaInstitucion<?= $sufijo ?>" id="busquedaInstitucion<?= $sufijo ?>" onchange="verPagina<?php echo $sufijo;?>(1)">
...
function verPagina<?php echo $sufijo;?>(pagina, loading=1){
	...
	let filtro='estado='+estado+'&nombre='+nombre+'&idinstitucion='+idinstitucion+'&nivel='+nivel;
	...
	setRun('presentacion/listaSucursal',filtro+extra+extrapermiso,'divLista<?php echo $sufijo;?>','divLista<?php echo $sufijo;?>',loading);	
}

verPagina<?php echo $sufijo;?>(1,0);

function NuevoRegistro<?= $sufijo ?>(){
	ViewModal('presentacion/mantSucursal','accion=NUEVO&idopcion=<?= $idopcion ?>&nivel=<?= $nivel + 1 ?>','divmodal<?= $nivel ?>','Registrar Sucursal',0);
}
