		...
		<div class="card card-custom gutter-b">
			<div class="card-header" style="min-height: 50px">
				<div class="card-title">
					...
					<small>Busqueda de Usuario</small></h3>
				</div>
			</div>
			<div class="card-body">
				<div class="row">
					...
					<div class="col-md-4">
						<div class="input-group input-group-sm">
							<div class="input-group-prepend">
								<span class="input-group-text font-weight-bolder">Perfil</span>
							</div>
							<select class="form-control" name="busquedaPerfil<?= $sufijo ?>" id="busquedaPerfil<?= $sufijo ?>" onchange="verPagina<?php echo $sufijo;?>(1)">
...
function verPagina<?php echo $sufijo;?>(pagina, loading=1){
	...
	let idperfil = $('#busquedaPerfil<?= $sufijo ?>').val();

	let filtro='estado='+estado+'&nombre='+nombre+'&idperfil='+idperfil;
	...
	setRun('presentacion/listaUsuario',filtro+extra+extrapermiso,'divLista<?php echo $sufijo;?>','divLista<?php echo $sufijo;?>',loading);	
}

verPagina<?php echo $sufijo;?>(1,0);

function NuevoRegistro<?= $sufijo ?>(){
	ViewModal('presentacion/mantUsuario','accion=NUEVO&idopcion=<?= $idopcion ?>','divmodal1','Registro de Usuario',0);
}
