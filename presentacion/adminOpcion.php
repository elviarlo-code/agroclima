...
$listaModulo = $objCase->getListTableFiltroSimple("opcion",'estado','N', 'idopcion_ref', 0);
$listaModulo = $listaModulo->fetchAll(PDO::FETCH_NAMED);
?>
<div class="row">
	<div class="col-md-12">
		<div class="card card-custom gutter-b">
			<div class="card-header" style="min-height: 50px">
				<div class="card-title">
					...
					<h3 class="card-label">Opciones
					<small>Busqueda de Opción</small></h3>
				</div>
			</div>
			<div class="card-body">
				<div class="row">
					<div class="col-md-4">
						<div class="input-group input-group-sm">
							<div class="input-group-prepend">
								<span class="input-group-text font-weight-bolder">Módulo</span>
							</div>
							<select class="form-control" name="busquedaModulo<?= $sufijo ?>" id="busquedaModulo<?= $sufijo ?>" onchange="verPagina<?php echo $sufijo;?>(1)">
								<option value="0">- Todos -</option>
								<?php foreach($listaModulo as $k=>$v){ ?>
								<option value="<?= $v['idopcion'] ?>"><?= $v['descripcion'] ?></option>
								<?php } ?>
							</select>
						</div>
					</div>
					<div class="col-md-4">
						<div class="input-group input-group-sm">
							<div class="input-group-prepend">
								<span class="input-group-text font-weight-bolder">Opción</span>
							</div>
					...
					<div class="col-md-4">
						<div class="input-group input-group-sm">
							<div class="input-group-prepend">
								<span class="input-group-text font-weight-bolder">Link</span>
							</div>
							<input type="text" name="busquedaLink<?= $sufijo; ?>" id="busquedaLink<?= $sufijo; ?>" class="form-control" onKeyUp="if(event.keyCode=='13'){ verPagina<?php echo $sufijo;?>(1); }" autocomplete="off">
						</div>
					</div>
					<div class="col-md-4">
						<div class="input-group input-group-sm">
...
function verPagina<?php echo $sufijo;?>(pagina, loading=1){
	...
	let link = $('#busquedaLink<?= $sufijo ?>').val();
	let modulo = $('#busquedaModulo<?= $sufijo ?>').val();

	let filtro='estado='+estado+'&descripcion='+descripcion+'&link='+link+'&modulo='+modulo;
	...
	setRun('presentacion/listaOpcion',filtro+extra+extrapermiso,'divLista<?php echo $sufijo;?>','divLista<?php echo $sufijo;?>',loading);	
}

verPagina<?php echo $sufijo;?>(1,0);

function NuevoRegistro<?= $sufijo ?>(){
	ViewModal('presentacion/mantOpcion','accion=NUEVO&idopcion=<?= $idopcion ?>','divmodal1','Registro de Opción',0);
}
